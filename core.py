...
重要：小红书不渲染 markdown。不要用 markdown 语法——不要用 # 或 ## 做标题，
不要用 ** 加粗，不要用 ``` 代码块。需要强调就用 emoji 或者换行分段，标题直接写文字。

但话题标签是例外：小红书的话题标签必须带 # 前缀，写成 #杭州citywalk #秋日穿搭
这种形式，这不是 markdown，是平台语法，一定要保留。
